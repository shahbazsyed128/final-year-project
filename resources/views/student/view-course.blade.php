                ...
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="p-2">
                            <form id="submitCommentForm">
                                @csrf
                                <input type="hidden" name="lecture_id" value="{{ $course['lectures'][0]['id'] }}">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="list-group list-group-flush ">
                            <a href="javascript:void(0)" class="list-group-item list-group-item-action active"> Quiz
                            ...
                            @forelse($course['quizzes'] as $key => $quiz)
                                <a href="{{ route('test', $quiz['id']) }}"
                                    ...
                                    {{ $quiz['title'] }}
                                    @if (isset($quiz['user_attempt'][0]['score']) && $quiz['user_attempt'][0]['score'] > $quiz['passing_marks'])
                                        <span class="float-right"><span
                                                class="text-success">{{ $quiz['user_attempt'][0]['score'] }}</span> /
                                            100</span>
                                    @elseif (isset($quiz['user_attempt'][0]['attempts']))
                                        <span class="float-right"><span
                                                class="text-danger">{{ $quiz['user_attempt'][0]['score'] }}</span> /
                                            100</span>
                                        @
                                    @endif
                                </a>
    ...
    @push('scripts')
        <script>
            ...
            $("[data-click='switch-video']").click(function() {
                ...
                $.get("{{ url('user/get-video-url') }}/" + lecture_id, function(response) {
                    try {
